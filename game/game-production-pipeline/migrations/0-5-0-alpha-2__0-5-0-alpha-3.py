#!/usr/bin/env python3
"""Plan the v0.5.0-alpha.2 to v0.5.0-alpha.3 migration."""
...
def build_migration(*, project_root: Path, source_root: Path, migration_at: str, from_version: str, from_lock: dict[str, Any]):
    return entry().build_alpha(project_root=project_root, source_root=source_root, migration_at=migration_at, from_version=from_version, from_lock=from_lock, expected_from_version=FROM_VERSION, supported_digests=SUPPORTED_FROM_FRAMEWORK_DIGESTS, metadata_only=True)
